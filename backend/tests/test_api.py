...
from database.database import Prediction, CounterInfo
...
def test_get_prediction_success(client: TestClient, db_session: Session):
    ...
    test_counter = CounterInfo(
        station_id="test-predict", name="Test Predict", longitude=1.0, latitude=1.0
    )
    ...
    response = client.get("/api/predict?station_id=test-predict")
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["station_id"] == "test-predict"
...
def test_get_prediction_not_found(client: TestClient):
    ...
    """
    response = client.get("/api/predict?station_id=non-existent-counter")
...
def test_trigger_daily_update(client: TestClient):
    ...
    with patch("api.endpoints.run_daily_update") as mock_run_update:
